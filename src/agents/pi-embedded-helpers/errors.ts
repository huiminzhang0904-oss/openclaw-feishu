...
export function formatRawAssistantErrorForUi(raw?: string): string {
  ...
  if (leadingStatus && isCloudflareOrHtmlErrorPage(trimmed)) {
    return `The AI service is temporarily unavailable (HTTP ${leadingStatus.code}). Please try again in a moment.\nAI 服务暂时不可用 (HTTP ${leadingStatus.code})，请稍后重试。`;
  }
...
const STATUS_CODE_MESSAGE_MAP = new Map<number, string>([
  ...
  [1400010721, "Claw Refuse content.\n小龙虾拒绝回答此问题，您可以修复重新询问其他问题"],
]);

function isStatusCodeErrorPayload(raw: string): boolean {
  return raw.includes("status_code") && raw.includes("status_message");
}

function extractStatusCodePayload(raw: string): Record<string, unknown> | null {
  // 1. 直接匹配顶层 JSON
  const directMatch = raw.match(/\{[^{}]*"status_code"\s*:\s*\d+[^{}]*"status_message"\s*:\s*"[^"]*"[^{}]*\}/);
  if (directMatch) {
    try {
  ...
  // 2. 嵌套场景：status_code 被转义在外层 JSON 的字符串值中
  const outerMatch = raw.match(/\{[\s\S]*\}/);
  if (outerMatch) {
    try {
      const outer = JSON.parse(outerMatch[0]) as Record<string, unknown>;
      const inner = typeof outer.error === "string" ? outer.error : "";
      if (inner.includes("status_code")) {
        const innerJsonMatch = inner.match(/\{[^{}]*"status_code"\s*:\s*\d+[^{}]*\}/);
        if (innerJsonMatch) {
          return JSON.parse(innerJsonMatch[0]) as Record<string, unknown>;
        }
      }
    } catch {
      // fall through
...
function formatStatusCodeError(raw: string): string | undefined {
  const parsed = extractStatusCodePayload(raw);
  if (!parsed) {
    return undefined;
  }
  const code = typeof parsed.status_code === "number" ? parsed.status_code : Number(parsed.status_code);
  const mappedMsg = STATUS_CODE_MESSAGE_MAP.get(code);
  if (mappedMsg) {
    return mappedMsg;
  }
  const msg = typeof parsed.status_message === "string" ? parsed.status_message.trim() : "";
  return msg || undefined;
}

// ── reply_map: keyword-based error rewriting (mirrors Python _get_apikey_error_reply) ──

const MODEL_CONFIG_CANDIDATE_PATHS = [
  "/workspace-inner/.openclaw/model_config.json",
  "/root/.openclaw/model_config.json",
];

const REPLY_MAP_CACHE_TTL_MS = 30_000;

let _replyMapCache: Record<string, string> | null = null;
let _replyMapCacheTime = 0;

/**
 * Load `reply_map` from the persisted `model_config.json`.
 *
 * The file is written by the Python-side `_model_config_sync_loop` (claw/service.py)
 * which periodically fetches the keyword→reply mapping from Apollo and persists it
 * alongside the model config. We cache the result for 30 s to avoid frequent disk reads.
 */
export function loadReplyMapFromModelConfig(): Record<string, string> | null {
  const now = Date.now();
  if (_replyMapCache !== null && now - _replyMapCacheTime < REPLY_MAP_CACHE_TTL_MS) {
    return _replyMapCache;
  }

  for (const candidatePath of MODEL_CONFIG_CANDIDATE_PATHS) {
    try {
      ...
      const replyMap = parsed.reply_map;
      if (replyMap && typeof replyMap === "object" && !Array.isArray(replyMap)) {
        _replyMapCache = replyMap as Record<string, string>;
        _replyMapCacheTime = now;
        return _replyMapCache;
      }
    } catch {
      // File missing, corrupt, or unreadable — try next candidate
    }
  }
...
/**
 * Match error text against a keyword→reply map (case-insensitive).
 *
 * Iterates over the map entries; if any keyword appears (case-insensitively) in
 * `errorText`, the corresponding reply string is returned. Returns `null` when
 * no keyword matches or the map is empty/undefined.
 */
export function matchErrorReplyMap(
  errorText: string,
  replyMap: Record<string, string> | undefined | null,
): string | null {
  if (!replyMap || !errorText) {
    return null;
  }
  const lower = errorText.toLowerCase();
  for (const [keyword, reply] of Object.entries(replyMap)) {
    if (keyword && String(keyword).trim() !== "" && lower.includes(String(keyword).toLowerCase())) {
      return String(reply);
    }
  }
  return null;
...
export function formatAssistantErrorText(
  ...
  const formatted = formatAssistantErrorTextInner(raw, opts);

  // Post-processing: apply keyword→reply overrides from model_config.json.
  // This mirrors the Python-side _get_apikey_error_reply in claw/service.py:
  // after the error has been classified and formatted, the already-formatted
  // text is matched against a keyword→reply map for further customization.
  const replyMap = loadReplyMapFromModelConfig();
  const rewritten = matchErrorReplyMap(formatted, replyMap);
  if (rewritten) {
    console.info(
      `[formatAssistantErrorText] error reply rewritten via reply_map: original=${JSON.stringify(formatted)}, rewritten=${JSON.stringify(rewritten)}`,
    );
    return rewritten;
  }
...
function formatAssistantErrorTextInner(
  raw: string,
  opts?: { cfg?: OpenClawConfig; sessionKey?: string; provider?: string; model?: string },
): string {
  const unknownTool =
  ...
  if (isContextOverflowError(raw)) {
    return (
      ...
      "上下文溢出：提示词对该模型来说过长。请尝试 /reset（或 /new）开始新会话，或使用支持更大上下文的模型。"
    );
  }
  ...
  if (
    ...
    return (
      ...
      "消息顺序冲突 - 请重试。如果问题持续，请使用 /new 开始新会话。"
    );
  }
  ...
  if (isMissingToolCallInputError(raw)) {
    return (
      ...
      "会话历史似乎已损坏（缺少工具调用输入）。请使用 /new 开始新会话。如果问题持续，请重置会话或删除损坏的会话记录。"
    );
  }

  const invalidRequest = raw.match(/"type":"invalid_request_error".*?"message":"([^"]+)"/);
  if (invalidRequest?.[1]) {
    return `LLM request rejected: ${invalidRequest[1]}\nLLM 请求被拒绝：${invalidRequest[1]}`;
  }
  ...
  if (isTimeoutErrorMessage(raw)) {
    return "LLM request timed out. The session is too long — try replying \"/new\" and wait a few minutes to recover.\n单次会话过长，请尝试回复\"/new\" 并等待几分钟进行恢复。";
  }
  ...
  if (isStatusCodeErrorPayload(raw)) {
    const statusCodeMsg = formatStatusCodeError(raw);
    if (statusCodeMsg) {

...
describe("Dockerfile", () => {
  ...
  it("copies feishu extension manifest before pnpm install", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
    const feishuCopyIndex = dockerfile.indexOf(
      "COPY --chown=node:node extensions/feishu/package.json ./extensions/feishu/package.json",
    );
    const installIndex = dockerfile.indexOf("pnpm install --frozen-lockfile");

    expect(feishuCopyIndex).toBeGreaterThan(-1);
    expect(installIndex).toBeGreaterThan(-1);
    expect(feishuCopyIndex).toBeLessThan(installIndex);
  });

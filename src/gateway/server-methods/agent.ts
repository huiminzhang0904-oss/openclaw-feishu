...
export const agentHandlers: GatewayRequestHandlers = {
  agent: async ({ params, respond, context, client, isWebchatConnect }) => {
    ...
    if (isAgentAbortCommand(message)) {
      ...
      if (!abortSessionKey) {
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.INVALID_REQUEST, "abort requires sessionKey or agentId"),
        );
        return;
      }
    ...
    const { signal: abortSignal } = createAgentAbortController(context.chatAbortControllers, {
      runId,
    ...
    void agentCommandFromIngress(
      ...
      .finally(() => {
        cleanupAgentAbortController(context.chatAbortControllers, runId);
      });
  },
